  ...
  capturePageSnapshot() {
    ...
    html2canvas(document.body, {
      ...
      ignoreElements: function (element) {
        // LOCAL PATCH (not upstream): html2canvas calls createPattern on any
        // <canvas> it meets, which throws InvalidStateError the moment one has
        // a zero width or height. This page has several WebGL canvases that are
        // legitimately 0x0 before they size themselves, and one throw aborts
        // the whole snapshot, so the glass never initialises at all. Canvases
        // cannot be rasterised usefully here anyway, so skip them.
